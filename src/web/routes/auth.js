...
// от сида. Роль явно не проверяется здесь: любая строка users (RUKOVODITEL
// или даже TP), заведённая вручную через /admin/users или созданная другими
// путями входа, тоже может войти этим путём при известном пароле -- это
// сознательно оставлено как запасной путь для админ-заведённых аккаунтов,
// не только для руководителя (QA PHA-82: предыдущая формулировка комментария
// ошибочно называла только RUKOVODITEL).
